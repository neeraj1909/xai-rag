...
| Python security gates | Pass for the default runtime | The locked default-runtime export has 0 known vulnerabilities. The optional `evaluation` extra is not approved for production because RAGAS 0.4.3 and its required pre-1.0 LangChain stack have unresolved advisories. |
...
| Offline and RAGAS evaluation | Strict versioned JSONL; 94 deterministic metrics; baseline comparison; explicit gates; current RAGAS collections metrics return faithfulness, answer relevancy, and reference/reference-free context precision when explicitly enabled | Replace synthetic smoke data with a reviewed golden set; automate pipeline capture; add confidence intervals and slice dashboards; calibrate model judges; move RAGAS to a patched dependency set before production use |
